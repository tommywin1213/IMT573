...
```{r}
#This code subsets the baseballData dataframe into a data frame that only contains relevant columns for our analysis
subset_baseball <- select(baseballData, c('Date','Home','Visitor','HomeLeague','VisitorLeague','HomeScore', 'VisitorScore'))
```
...
```{r}
#This code creates two new columns that indicates whether the home team or visiting team won. It then uses that column in conjunction with the score to determine the conditional probability
subset_baseball <- subset_baseball %>% mutate(home_win=ifelse(HomeScore > VisitorScore,T,F))
subset_baseball <- subset_baseball %>% mutate(visitor_win=ifelse(VisitorScore > HomeScore,T,F))

#This code subsets the data further by taking out all games where the visiting team won 
home_wins <- filter(subset_baseball, home_win ==  TRUE)
home_total <- as.integer(count(home_wins))

#This code subsets the data by only including games where the visitng team has won 
visitor_wins <- filter(subset_baseball, visitor_win ==  TRUE)
visitor_total <- as.integer(count(visitor_wins))
