...
#### (a) To begin load the \texttt{fatherson.csv.bz2} data. Create density plots of both heights on the same figure.  Comment the plots.  What do they look like?  What do they suggest in terms of fathers' and sons' relative height?
```{r}
#This code reads the csv file, it then creates a plot using ggplot 
fathers_sons <- read.csv(file = "data/fatherson.csv.bz2", head = TRUE, sep='\t')

p <- ggplot() +
      geom_density(data=fathers_sons, aes(x=fheight)) + 
      geom_density(data=fathers_sons, aes(x=sheight)) 

print(p)

```
This visual suggests that a sons height is largely correlated to their fathers height 
...
```{r}
sample_mean <- mean(fathers_sons$fheight)
assumed_mean <- ((mean(fathers_sons$fheight)+mean(fathers_sons$sheight))/2)
standard_dev <- sd(fathers_sons$fheight)
n <- sqrt(nrow(fathers_sons))

t <- ((sample_mean - assumed_mean) / (standard_dev/n))
```
...
Based on my analysis, it seems that there is a correlation where sons are taller than their fathers. 
...
```{r}
total_mean <- colMeans(fathers_sons[,c('fheight','sheight')], na.rm = TRUE)
```
The mean for both is 173, while the mean for fathers is 172 and for sons is 174. 
