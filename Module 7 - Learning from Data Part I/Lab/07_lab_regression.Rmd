...
download.file("https://www.openintro.org/book/statdata/index.php?data=nycflights", destfile = "data/flights.RData")
...
```
In general, there does seem to be a linear correlation between the number of runs and bats. As the number of at bats increase, the number of runs increases as well. 

* If you knew a team's at bats, would you be comfortable using a linear model to predict the number of runs?
Yes, I would be comfortable using the linear model to predict the number of runs.
...
```
A correlation coefficient of .6 indicates that there is a significant positive relationship between the two variables. 
...
Beta 1 is essentially the slope of the data, which can be interpreted through the r squared value.
...
The fit of the model supports the argument that there is a correlation between runs and at bats. 